...
function contactFrames(speed, capTop, hz) {
  ...
  return { maxRun, duty: (hitFrames / total * 100).toFixed(1) + '%', maxTrauma: maxTrauma.toFixed(2), passes: runs.length, avgRun: runs.length ? (runs.reduce((a, b) => a + b) / runs.length).toFixed(1) : 0 }
}

const spots = [
  ['站风道底板630 cap542..630', 542],
  ['半跳中 cap470..558', 470],
  ['轴心高度 cap356..444', 356],
]
for (const hz of [165, 60]) {
  for (const [name, top] of spots) {
    for (const [mode, sp] of Object.entries(d.speeds)) {
      ...
      console.log(`${hz}Hz | ${mode.padEnd(4)} | ${name.padEnd(26)} 连续命中帧max=${String(r.maxRun).padStart(3)} 均=${String(r.avgRun).padStart(5)} 扫过次数/10s=${String(r.passes).padStart(3)} 占空比=${r.duty.padStart(6)} trauma峰=${r.maxTrauma}`)
    }
  }
  console.log('')
}
console.log('单次 hurt=16, hp=100, 700ms 无敌 → 持续接触 10s 内最多 ' + Math.floor(10000 / 700) * 16 + ' 伤害')
